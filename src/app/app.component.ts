...
import { from, interval, timer, fromEvent, merge, empty, Observable, of, range, BehaviorSubject, Subject, NEVER } from 'rxjs';
import { switchMap, scan, takeWhile, startWith, mapTo, tap, map, takeUntil } from 'rxjs/operators';
...
export class AppComponent implements OnInit {
  ...
  startTimer() {
    ...
    this.timer$ = merge(this.pause$)
      ...
      tap(x => x === 31 || x === 30 || x === 29 ? this.beep() : () => { }),
      tap(x => x <= 5 ? this.beep() : () => { }),
      tap(x => x === 0 ? this.next() : () => { }),
      map(x => x * 1000),
      takeWhile(v => v >= 0)
